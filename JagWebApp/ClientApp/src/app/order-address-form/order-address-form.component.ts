...
import { ERROR_PATTERN_MESSAGE, ERROR_REQUIRED_MESSAGE, SUCCESS_SAVE_DATA_MESSAGE } from '../consts/user-messages.consts';
import { notEmptyInput } from '../validators/not-empty-input.validator';
...
export class OrderAddressFormComponent implements OnInit {
  ...
  getCustomerNameErrorMessage() {
    return this.customerName.hasError('required') || this.customerName.hasError('empty') ? ERROR_REQUIRED_MESSAGE : '';
  }

  getPhoneErrorMessage() {
    return this.phone.hasError('required') ? ERROR_REQUIRED_MESSAGE 
      : this.phone.hasError('pattern') ? ERROR_PATTERN_MESSAGE 
        : '';
  }
  ...
  getHouseNumberErrorMessage() {
    return this.houseNumber.hasError('required') || this.houseNumber.hasError('empty') ? ERROR_REQUIRED_MESSAGE : '';
  }

  getPostcodeErrorMessage() {
    return this.postcode.hasError('required') ? ERROR_REQUIRED_MESSAGE
      : this.postcode.hasError('pattern') ? ERROR_PATTERN_MESSAGE 
        : '';
  }
